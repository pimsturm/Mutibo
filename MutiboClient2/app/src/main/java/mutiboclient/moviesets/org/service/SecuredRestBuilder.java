...
public class SecuredRestBuilder extends RestAdapter.Builder {
			...
			if (!loggedIn) {
				try {
					...
					if (resp.getStatus() < 200 || resp.getStatus() > 299) {
                        ...
                        SecuredRestBuilder.this.accessToken = accessToken;
	...
	public RestAdapter build() {
		...
		OAuthHandler hdlr = getOAuthHandler();
        if (hdlr == null) {
    ...
    private OAuthHandler getOAuthHandler(){
        if (accessToken != null) {
            return new OAuthHandler(client, loginUrl, accessToken);
        }

        if (username != null && password != null) {
            return new OAuthHandler(client, loginUrl, username, password, clientId, clientSecret);
        }
